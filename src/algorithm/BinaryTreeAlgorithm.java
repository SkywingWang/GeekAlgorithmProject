...
public class BinaryTreeAlgorithm {
    ...
    public List<List<Integer>> findCriticalAndPseudoCriticalEdges(int n, int[][] edges) {
        ...
        UnionFindFCAPCE unionFindFCAPCE = new UnionFindFCAPCE(n);
        int value = 0;
        for(int i = 0; i < m; i++){
            if(unionFindFCAPCE.unite(newEdges[i][0],newEdges[i][1])){
                value += newEdges[i][2];
            }
        }
        List<List<Integer>> ans = new ArrayList<>();
        ...
        for(int i = 0; i < m; i++){
            UnionFindFCAPCE uf = new UnionFindFCAPCE(n);
            int v =0;
            for(int j = 0; j < m; j++){
    ...
    private class UnionFindFCAPCE{
        int[] parent;
        int[] size;
        int n;
        int setCount;

        public UnionFindFCAPCE(int n){
            this.n = n;
            this.setCount = n;
